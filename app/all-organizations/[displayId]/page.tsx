...
import { organizations, contacts, proposals, agencyClients, agencies } from "@/lib/db/schema";
import { normalizeWebsiteUrl } from "@/lib/normalize-website-url";
import { buildOrganizationPlainText } from "@/lib/plain-text-profile";
...
export default async function OrganizationDetailPage({ params }: Props) {
  ...
  const [agencyLink] = await db
    ...
    .where(
      and(eq(agencyClients.companyType, "org"), eq(agencyClients.companyDisplayId, displayId))
    )
    .limit(1);
  ...
  const plainText = buildOrganizationPlainText({
    org,
    transactions,
    moneySpentRaw,
    agency: agencyLink
      ? { agencyName: agencyLink.agencyName, agencyDisplayId: agencyLink.agencyDisplayId }
      : null,
        ...
        >
          ...
          {agencyLink && (
            <div style={infoStyle}>
              ...
              {" "}
              <Link
                href={`/new-agency?linkCompany=${encodeURIComponent(displayId)}&linkType=org&fromAgency=${encodeURIComponent(agencyLink.agencyDisplayId ?? "")}`}
                style={{
                  fontSize: "0.8rem",
