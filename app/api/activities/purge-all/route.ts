...
import { activities, sessions, users } from "@/lib/db/schema";
...
/** POST: Purge all activity records. */
export async function POST(req: NextRequest) {
  try {
    ...
    console.error("[api/activities/purge-all POST]", err);
    return NextResponse.json({ error: "Failed to purge activity" }, { status: 500 });
  }
}
