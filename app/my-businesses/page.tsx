...
export default function MyBusinessesPage() {
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      >
        <h1 style={{ color: "var(--gold-bright)" }}>My Businesses</h1>
        <Link
          href="/new-business"
          style={{
              ...
              {list.map((b) => (
                ...
                >
                  <td style={tdStyle}>
                    {b.lastActivityAt ? (
                      <>
                        <div>{formatDateTime(b.lastActivityAt)}</div>
                        {b.lastActivityType && <div style={{ fontSize: "0.75rem", color: "var(--gold-dim)" }}>{b.lastActivityType}</div>}
                      </>
                    ) : (
                      "—"
                    )}
                  ...
                  <td style={tdStyle}>{b.businessType ?? "—"}</td>
                  <td style={tdStyle}>{b.tags ?? "—"}</td>
                  <td style={tdStyle}>{b.displayId ?? "—"}</td>
                  <td style={tdStyle}>
                    <button type="button" onClick={() => openEdit(b)} style={btnSmall}>Edit</button>
                  </td>
                </tr>
              ))}
            </tbody>
